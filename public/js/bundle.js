(() => {
  ...
  var displayMap = (locations) => {
    ...
    const greenIcon = L.icon({
      iconUrl: "/img/pin.png",
      // the script runs from tour.pug
      iconSize: [32, 40],
      // size of the icon
      iconAnchor: [16, 45],
      // point of the icon which will correspond to marker's location
      popupAnchor: [0, -50]
      // point from which popup should open in relative to the iconAnchor
    });
    const points = [];
    locations.forEach((loc) => {
      ...
      L.marker([loc.coordinates[1], loc.coordinates[0]], { icon: greenIcon }).addTo(map).bindPopup(`<p>Day ${loc.day}: ${loc.description}</p>`, {
        autoClose: false,
        // prevents closing this popup when another opens
        closeOnClick: false,
        // prevents closing the popup when the map is clicked
        className: "mapPopup"
      }).openPopup();
    });
